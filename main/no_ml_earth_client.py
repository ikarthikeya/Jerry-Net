"""
machine learning prediction and earth to satellite handover logic: Daim
latency simulation based on satellite distance: Ting
"""
import subprocess
import socket
import os
...
def start_clumsy(single_latency, single_drop_rate):
    if os.name == "nt":
        # if windows system then use clumsy
        ...
        cmd = f"tc qdisc add dev eth0 root netem delay {single_latency}ms loss {single_latency}%"
...
def client(routing_manager, sat_addresses, receiver_ports, earth2_ll, buffer_size, timeout, debug_inter, chunk_size,
    ...
    earth2_addr = ['127.0.0.1', 50021]  # earth2 addr
